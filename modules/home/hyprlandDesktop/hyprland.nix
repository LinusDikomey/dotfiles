{pkgs}: {
  ...
  settings = {
    monitor = [
      ...
      "HDMI-A-5, 1920x1080@60, 2560x720, 1"
  ...
  extraConfig = ''
    bind = $mod, Y, submap, mc

    submap = mc
    bind =           , grave, pass, ^(HyprMCSR)$
    bind =           , P    , pass, ^(HyprMCSR)$
    bind = SHIFT     , G    , pass, ^(HyprMCPR)$
    bind =           , F3 C , pass, ^(Ninjabrain Bot)$

    bind = $mod SHIFT, Y    , submap, reset
    submap = reset
  '';
}
